...
def verifica_despesa(nome_tabela,ano,valor_pesquisa):
        ...
        caminho = f"{desktop_path}\\TABELAS\\{ano}\\{nome_tabela}.xls"
                    ...
                    if pd.isna(valor_2):
                        if pd.isna(valor_5):
                            if pd.isna(valor_7):
                                if pd.isna(valor_9):
                                    if pd.isna(valor_10):
                                        total_despesa += valor_11
                                    #print(f"{linha.iloc[1]} - R$ {valor_10}")
                                        arr.append({"nome_dado": linha.iloc[1],
                                                "valor_dado": f"R$ {valor_11:,.2f}".replace(',', 'TEMP').replace('.', ',').replace('TEMP', '.')})
                                    else:
